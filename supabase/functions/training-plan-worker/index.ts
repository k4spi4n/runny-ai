...
import { processPendingTrainingPlanJobs } from "../_shared/training_plan.ts";
...
serve(async (req) => {
  ...
  const expected = Deno.env.get("TRAINING_PLAN_WORKER_TOKEN")?.trim();
  ...
  try {
    ...
    const limit = typeof rawLimit === "number" && Number.isInteger(rawLimit)
      ? Math.max(1, Math.min(5, rawLimit))
      : 2;
    const processed = await processPendingTrainingPlanJobs(limit);
    return jsonResponse(
      req,
      { processed },
      200,
    ...
    console.error(JSON.stringify({
      event: "training_plan_worker_failed",
