...
export class GroqFoodRecognitionService implements FoodRecognitionService {
  ...
  async analyze(image: FoodImageInput): Promise<FoodRecognitionResult> {
    ...
    if (!res.ok) {
      ...
      console.error(`Groq vision returned ${res.status}: ${errText}`);
    ...
    const isFood = parsed.is_food !== false; // mac dinh coi la mon an tru khi model phu nhan
    ...
    return {
      ...
      nutrition: {
        calories: Math.round(clampNumber(nutritionRaw.calories, 0, MAX_CALORIES)),
        protein: Math.round(clampNumber(nutritionRaw.protein, 0, MAX_MACRO_GRAMS)),
...
export function createFoodRecognitionService(): FoodRecognitionService {
  ...
  // Mac dinh: dung Groq khi co key, nguoc lai mock. Ep mock bang FOOD_RECOGNITION_PROVIDER=mock.
  const provider = Deno.env.get('FOOD_RECOGNITION_PROVIDER') ?? (groqKey ? 'groq' : 'mock');

  if (provider === 'groq') {
    if (!groqKey) {
      console.warn('FOOD_RECOGNITION_PROVIDER=groq nhung thieu GROQ_API_KEY. Dung mock.');
      return new MockFoodRecognitionService();
    }
    const model = Deno.env.get('FOOD_RECOGNITION_MODEL') ?? GROQ_DEFAULT_VISION_MODEL;
    return new GroqFoodRecognitionService(groqKey, model);
  }

  if (provider !== 'mock') {
    console.warn(`FOOD_RECOGNITION_PROVIDER=${provider} khong duoc ho tro. Dung mock.`);
  }
  return new MockFoodRecognitionService();
}
